...
class MyDigitizationAlgorithm final : public IAlgorithm {
 public:
  class Config {
   public:
  ...
  MyDigitizationAlgorithm(Config config, Acts::Logging::Level level);
 ...
 private:
  ...
  WriteDataHandle<IndexMultimap<SimBarcode>> m_outputMeasurementParticlesMap{this, "OutputMeasurementParticlesMap"};
  WriteDataHandle<IndexMultimap<Index>> m_outputMeasurementSimHitsMap{this, "OutputMeasurementSimHitsMap"};
  WriteDataHandle<InverseMultimap<SimBarcode>> m_outputParticleMeasurementsMap{this, "OutputParticleMeasurementsMap"};
  WriteDataHandle<InverseMultimap<Index>> m_outputSimHitMeasurementsMap{this, "OutputSimHitMeasurementsMap"};
  ...
  Digitizer makeDigitizer(const DigiComponentsConfig& cfg) {
    ...
    for (std::size_t i = 0; i < kSmearDIM; ++i) {
      ...
      impl.smearing.smearFunctions[i] = cfg.smearingDigiConfig.params.at(i).smearFunction;
      impl.smearing.forcePositive[i] = cfg.smearingDigiConfig.params.at(i).forcePositiveValues;
      impl.smearing.maxRetries = std::max(m_cfg.minMaxRetries, cfg.smearingDigiConfig.maxRetries);
    }
    return impl;
  }
};
